...
import { Check, Edit3, Maximize, Minimize, Loader2, EyeIcon, CodeIcon } from "lucide-react";
...
const MarkdownContent: React.FC<{ content: string; isFullScreen?: boolean; contentWrapperId?: string }> = ({ content, isFullScreen, contentWrapperId }) => {
  ...
  let htmlContent = content
    ...
    .replace(/^# (.*$)/gim, `<h2 class="text-lg sm:text-xl font-bold mt-4 mb-2 ${isFullScreen ? 'text-foreground' : 'text-primary'} underline underline-offset-4 decoration-primary/50">${'$1'}</h2>`)
...
export function ReadmeDisplay({ data, onGenerateDetails, isGeneratingDetails, onEditRequest }: ReadmeDisplayProps) {
  ...
  useEffect(() => {
    if (!mounted || !data || viewMode === 'raw') return; // Don't attach if in raw mode or not mounted
  ...
  }, [data, isFullScreen, mounted, toast, contentWrapperId, viewMode]); // Add viewMode to dependencies
      ...
      <CardContent className={cn(
        ...
        <ScrollArea 
          ...
          className={cn(
            ...
            isFullScreen ? "flex-1 h-auto w-full border-0 rounded-none bg-transparent" : "h-[calc(100vh-360px)] min-h-[300px] sm:h-[calc(100vh-320px)] md:h-[500px] rounded-b-lg"
          )}
        >
        {viewMode === 'formatted' ? (
          <div id={contentWrapperId} className={cn( 
            ...
            {!isFullScreen && (
              <div className="py-1.5 sm:py-2 border-b border-border/50">
                <h1 className="text-base sm:text-lg font-bold text-primary underline decoration-primary/60 underline-offset-2 sm:underline-offset-4 mb-1 sm:mb-1.5 font-headline">
                  {data.projectName}
                </h1>
              </div>
            )}
            ...
            <pre
              className={cn(
                ...
                isFullScreen ? "max-w-4xl mx-auto p-3 py-3 sm:p-6 sm:py-4 h-full" : "p-3 sm:p-4 m-3 sm:m-4" // Apply margin when not fullscreen
              )}
            >
