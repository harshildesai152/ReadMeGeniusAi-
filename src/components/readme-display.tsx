...
const MarkdownContent: React.FC<{ content: string; isFullScreen?: boolean; contentWrapperId?: string }> = ({ content, isFullScreen, contentWrapperId }) => {
  ...
  let htmlContent = content
    ...
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/^### (.*$)/gim, `<h4 class="text-sm sm:text-base font-semibold mt-2 mb-1 ${isFullScreen ? 'text-foreground/90' : 'text-primary/90'}">${'$1'}</h4>`)
     ...
     <Card className={cn(
      ...
      <CardContent className={cn(
        ...
        <ScrollArea ref={scrollAreaRef} className={cn(
          "h-[calc(100vh-360px)] min-h-[300px] sm:h-[calc(100vh-320px)] md:h-[500px] w-full rounded-b-lg bg-background",
          isFullScreen && "flex-1 h-auto w-full border-0 rounded-none bg-transparent" 
          )}>
          <div id={contentWrapperId} className={cn( // Add ID here for DOM manipulation scope
