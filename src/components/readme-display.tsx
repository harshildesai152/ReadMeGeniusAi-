...
const MarkdownContent: React.FC<{ content: string; isFullScreen?: boolean }> = ({ content, isFullScreen }) => {
  if (!content && content !== "") return <p className="text-muted-foreground italic text-sm sm:text-base">Not available or empty.</p>;
  ...
  let htmlContent = content
    ...
    .replace(/^(?:(?:- |\* |\+ )\s*.*(?:\n|$))+/gm, (match) => {
      const items = match.trim().split('\n').map(item => `<li class="ml-5 sm:ml-6 list-disc space-y-0.5 my-0.5 sm:my-1 text-xs sm:text-sm ${isFullScreen ? 'text-foreground/90' : 'text-foreground/90'}">${item.replace(/^(- |\* |\+ )\s*/, '')}</li>`).join('');
      return `<ul class="space-y-0.5 mb-1.5 sm:mb-2">${items}</ul>`;
    })
    .replace(/^(?:\d+\.\s*.*(?:\n|$))+/gm, (match) => {
        const items = match.trim().split('\n').map(item => `<li class="ml-5 sm:ml-6 list-decimal space-y-0.5 my-0.5 sm:my-1 text-xs sm:text-sm ${isFullScreen ? 'text-foreground/90' : 'text-foreground/90'}">${item.replace(/^\d+\.\s*/, '')}</li>`).join('');
        return `<ol class="space-y-0.5 mb-1.5 sm:mb-2">${items}</ol>`;
    })
    .replace(/```([\s\S]*?)```/g, (match, p1) => `<pre class="bg-muted/80 p-2.5 sm:p-3.5 rounded-md text-xs sm:text-sm overflow-x-auto my-1.5 sm:my-2.5 font-mono shadow-md border border-border/70 ${isFullScreen ? 'text-foreground/90' : 'text-foreground/90'}">${p1.trim().replace(/</g, "&lt;").replace(/>/g, "&gt;")}</pre>`)
    .replace(/\n/g, '<br />');
  ...
  htmlContent = htmlContent.split('<br />').map(line => {
    ...
    return `<p class="mb-2 sm:mb-2.5 leading-relaxed text-xs sm:text-sm ${isFullScreen ? 'text-foreground/90' : 'text-foreground/90'}">${line}</p>`;
  }).join('<br />').replace(/<br \/>(<p)/g, '$1').replace(/(<\/p>)<br \/>/g, '$1');


  return <div className={cn("prose dark:prose-invert max-w-none", isFullScreen ? "text-sm sm:text-base" : "text-xs sm:text-sm")} dangerouslySetInnerHTML={{ __html: htmlContent || '' }} />;
};
  ...
  return (
    <Card className={cn(
        ...
        <div className={cn(
          ...
          >
            {isGeneratingDetails ? (
              ...
              <FileTextIcon className="mr-1 h-3.5 w-3.5 sm:h-4 sm:w-4" />
            )}
          ...
          >
            {isCopied ? <Check className="mr-1 h-3.5 w-3.5 sm:h-4 sm:w-4" /> : <Copy className="mr-1 h-3.5 w-3.5 sm:h-4 sm:w-4" />}
      ...
      <CardContent className={cn(
         isFullScreen && "flex-1 flex flex-col min-h-0 p-0" 
        )}>
          ...
          <div className={cn(
            ...
            {!isFullScreen && (
              <div className="py-1.5 sm:py-2 border-b border-border/50">
                <h2 className="text-base sm:text-lg font-bold text-primary underline decoration-primary/60 underline-offset-2 sm:underline-offset-4 mb-1.5 sm:mb-2 font-headline">
                  1. Project Name:
                </h2>
            ...
            <div className={cn(!isFullScreen && "py-1.5 sm:py-2 border-b border-border/50")}>
              {!isFullScreen && (
                <h2 className="text-base sm:text-lg font-bold text-primary underline decoration-primary/60 underline-offset-2 sm:underline-offset-4 mb-1.5 sm:mb-2 font-headline">
                  {isFullScreen ? "" : "2. Project Description:"}
                </h2>
              )}
               {isFullScreen && <h1 className="text-xl sm:text-2xl font-bold text-primary mb-2.5 sm:mb-3 mt-1 sm:mt-1.5">{data.projectName}</h1>}
              <MarkdownContent content={data.projectDescription} isFullScreen={isFullScreen} />
            </div>

            <div className={cn(!isFullScreen && "py-1.5 sm:py-2 border-b border-border/50")}>
              <h2 className={cn(
                !isFullScreen && "text-base sm:text-lg font-bold text-primary underline decoration-primary/60 underline-offset-2 sm:underline-offset-4 mb-1.5 sm:mb-2 font-headline",
                isFullScreen && "text-lg sm:text-xl font-semibold text-primary mt-3 sm:mt-4 mb-1.5 sm:mb-2"
              )}>
            ...
            <div className={cn(!isFullScreen && "py-1.5 sm:py-2 last:border-b-0")}>
              <h2 className={cn(
                !isFullScreen && "text-base sm:text-lg font-bold text-primary underline decoration-primary/60 underline-offset-2 sm:underline-offset-4 mb-1.5 sm:mb-2 font-headline",
                 isFullScreen && "text-lg sm:text-xl font-semibold text-primary mt-3 sm:mt-4 mb-1.5 sm:mb-2"
              )}>
